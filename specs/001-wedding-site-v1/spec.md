...
- 英文裝飾元素統一使用 `Great Vibes, cursive`（`font-script` 與 `font-display` 共用同一字型堆疊），不受 `.wedding-site` scope 影響
...
**字體規範**：
英文文字 MUST 統一使用 `'Great Vibes', cursive`（`font-script` 與 `font-display` 共用同一字型堆疊），用於大標題、區塊標題、年份標注、情感短語及所有英文裝飾文字。
中文內容 MUST 使用 `'Iansui', 'Noto Serif TC', serif`（本地字型檔 `src/assets/fonts/Iansui-Regular.ttf`，CSS 變數 `--font-wedding-zh`，Tailwind token `font-wedding-zh`），透過 `.wedding-site` CSS scope 涵蓋所有前台中文文字。
...
- **字型**：英文 MUST 統一使用 `'Great Vibes', cursive`（`font-script` = `font-display`，不再使用 Cormorant Garamond / Playfair Display）；中文 MUST 使用 `'Iansui', 'Noto Serif TC', serif`（`font-wedding-zh`，CSS var `--font-wedding-zh`），透過 `.wedding-site` scope class 自動套用至所有前台頁面（`/admin` 除外）
