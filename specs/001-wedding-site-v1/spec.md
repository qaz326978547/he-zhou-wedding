...
本規格定義 HE Bean & Katherine Zhou 婚禮邀請網站 v1 的完整功能範疇。
...
1. **Given** 賓客點擊 CTA 按鈕或滾動至 RSVP 表單，**When** 表單顯示，**Then** 看到姓名、電話、出席狀態（出席/不克出席）、出席人數、賓桌歸屬（新郎方/新娘方，選填）、飲食偏好（一般/素食/不吃牛肉/不吃豬肉/其他，選填）、備註（選填，最多 300 字元）等欄位；賓桌歸屬下方初始不顯示關係類型。
...
7. **Given** 賓客選擇賓桌歸屬（如「新郎方」），**When** 選擇後，**Then** 表單在歸屬選項下方動態顯示關係類型選項（親屬 / 朋友）；若賓客未選擇歸屬，關係類型選項 MUST NOT 顯示。
...
- `font-display`（Cormorant Garamond）與 `font-script`（Great Vibes）保留用於英文裝飾元素，不受 `.wedding-site` scope 影響
...
**字體規範**：
系統 SHOULD 使用手寫英文字體（如 Great Vibes、Pinyon Script、Alex Brush 或同等書法體字型），
用於大標題、區塊標題、年份標注、情感短語等視覺重點位置，傳達浪漫氛圍。
正文 MUST 使用極簡細字體（Cormorant Garamond、Playfair Display 或同等精緻 serif 字型），
維持 Premium Editorial 排版感。
中文內容 MUST 使用 Iansui（芫荽）字型（本地字型檔 `src/assets/fonts/Iansui-Regular.ttf`），CSS 變數 `--font-wedding-zh` 與 Tailwind token `font-wedding-zh` 統一管理，透過 `.wedding-site` CSS scope 涵蓋所有前台中文文字。
...
- **RSVP 回覆（RSVPSubmission）**：賓客出席回覆紀錄；包含姓名、電話（唯一識別）、出席狀態（Boolean）、出席人數（attending=true 時 1–10，attending=false 時固定為 0）、賓桌歸屬（`relationshipSide`，選填：新郎方/新娘方）、關係類型（`relationshipType`，選填：親屬/朋友，僅在 relationshipSide 有值時有效）、飲食偏好、備註（最多 300 字元）、提交時間；另含通知信發送狀態欄位（sent、sentAt、error）
...
`relationshipSide` 允許值：`groom`（新郎方）、`bride`（新娘方）、`null`（未填寫）。
...
- **字型**：英文大標題 / 手寫情感字使用 Great Vibes（`font-script`）；英文正文標題使用 Cormorant Garamond / Playfair Display（`font-display`，精緻 serif）；中文 MUST 使用 **Iansui（芫荽）**（本地字型，CSS var `--font-wedding-zh`，Tailwind token `font-wedding-zh`），透過 `.wedding-site` scope class 自動套用至所有前台頁面（`/admin` 除外）；中文主姓名 SHOULD NOT 使用手寫字體，以維持可讀性
