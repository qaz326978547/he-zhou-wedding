<template>
  <div class="min-h-screen bg-gray-50">
    <!-- Header -->
    <div
      class="bg-white border-b px-4 py-3 flex items-center justify-between sticky top-0 z-10"
    >
      <h1 class="text-lg font-semibold text-gray-800">RSVP 管理後台</h1>
      <div class="flex items-center gap-3">
        <RouterLink
          to="/admin/red-envelope"
    ...
    <div class="max-w-7xl mx-auto px-4 py-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-2">
              ...
              <div v-if="editForm.needsHighchair === true">
                ...
                <select v-model="editForm.highchairCount" class="edit-input">
                  <option
                    v-for="n in editForm.childCount || 0"
                    :key="n"
                    :value="n"
                  >
                    {{ n }} 張
              ...
              <div>
                ...
                <select
                  ...
                  class="edit-input"
                  :disabled="!editForm.relationshipSide"
                >
                  <option value="">--</option>
              ...
              <template v-if="editForm.needsInvitation">
                <div class="col-span-2">
                  <label class="text-xs text-gray-400">收件人</label
                  ><input
                    v-model="editForm.invitationName"
                    class="edit-input"
                  />
                </div>
                <div>
                  <label class="text-xs text-gray-400">收件電話</label
                  ><input
                    v-model="editForm.invitationPhone"
                    class="edit-input"
                  />
                </div>
                <div>
                  <label class="text-xs text-gray-400">收件地址</label
                  ><input
                    v-model="editForm.invitationAddress"
                    class="edit-input"
                  />
                </div>
              </template>
          ...
          <template v-else>
            ...
            <div class="text-sm text-gray-500 space-y-0.5">
              ...
              <div v-if="item.childCount > 0">
                兒童椅：{{
                  item.needsHighchair === true
                    ? `需要 ${item.highchairCount ?? 1} 張`
                    : item.needsHighchair === false
                      ? "不需要"
                      : "--"
                }}
              </div>
              <div v-if="item.relationshipSide">
                賓桌：{{
                  item.relationshipSide === "groom" ? "新郎親友" : "新娘親友"
                }}
              ...
              <div v-if="item.dietaryPreference === 'vegetarian'">
                飲食：素食
              </div>
              <div>
                紙本喜帖：{{ item.needsInvitation ? "需要" : "不需要" }}
              </div>
              <template v-if="item.needsInvitation">
                <div class="text-xs text-gray-400">
                  收件：{{ item.invitationName }} · {{ item.invitationPhone }}
                </div>
                <div class="text-xs text-gray-400 truncate">
                  {{ item.invitationAddress }}
                </div>
              </template>
              <div class="text-xs text-gray-400">
                通知信：{{ item.notificationEmailSent ? "已發送" : "未發送" }}
              </div>
            </div>
      ...
      <!-- Desktop table (>= md) -->
      <div
        class="hidden md:block bg-white rounded-xl shadow-sm overflow-x-auto"
      >
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            >
              <template v-if="editingId === item.id">
                <td class="px-2 py-2">
                  <input v-model="editForm.name" class="edit-input w-24" />
                </td>
                <td class="px-2 py-2">
                  <input v-model="editForm.phone" class="edit-input w-32" />
                </td>
                <td class="px-2 py-2">
                ...
                <td class="px-2 py-2">
                  <select
                    v-model="editForm.needsHighchair"
                    class="edit-input w-20"
                    :disabled="!editForm.childCount"
                  >
                    <option :value="null">--</option>
                    <option :value="true">需要</option>
                    <option :value="false">不需要</option>
                  ...
                  <select
                    v-if="editForm.needsHighchair === true"
                    v-model="editForm.highchairCount"
                    class="edit-input w-16 mt-1"
                  >
                    <option
                      v-for="n in editForm.childCount || 0"
                      :key="n"
                ...
                <td class="px-2 py-2">
                  <select
                    v-model="editForm.relationshipSide"
                    class="edit-input w-24"
                  >
                    <option value="">--</option>
                    <option value="groom">新郎親友</option>
                    <option value="bride">新娘親友</option>
                ...
                <td class="px-2 py-2">
                  <select
                    v-model="editForm.dietaryPreference"
                    class="edit-input w-20"
                  >
                    <option value="regular">葷食</option>
                    <option value="vegetarian">素食</option>
                ...
                <td class="px-2 py-2">
                  <select
                    v-model="editForm.needsInvitation"
                    class="edit-input w-20"
                  >
                    <option :value="false">不需要</option>
                    <option :value="true">需要</option>
                ...
                <td class="px-2 py-2 whitespace-nowrap">
                  <p v-if="editError" class="text-red-500 text-xs mb-1">
                    {{ editError }}
                  </p>
              ...
              <template v-else>
                ...
                <td class="px-4 py-3 text-gray-600">
                  {{
                    item.relationshipSide === "groom"
                      ? "新郎親友"
                      : item.relationshipSide === "bride"
                        ? "新娘親友"
                        : "--"
                  }}
                </td>
                <td class="px-4 py-3 text-gray-600">
                  {{
                    item.relationshipType === "relative"
                      ? "親戚"
                ...
                <td class="px-4 py-3 text-gray-600">
                  {{
                    item.dietaryPreference === "vegetarian" ? "素食" : "葷食"
                  }}
                </td>
                <td class="px-4 py-3 text-gray-600">
                  <span v-if="item.needsInvitation" class="text-blue-600"
                    >需要</span
                  >
...
const sideTabs = [
  ...
  { value: "groom" as const, label: "新郎親友" },
  { value: "bride" as const, label: "新娘親友" },
];
...
async function saveEdit(id: number) {
  ...
  try {
    const payload: any = {
      ...
      adultCount:
        editForm.value.adultCount === ""
          ? null
          : Number(editForm.value.adultCount),
      childCount: Number(editForm.value.childCount),
      needsHighchair:
        editForm.value.childCount > 0 ? editForm.value.needsHighchair : null,
      highchairCount:
        editForm.value.childCount > 0 && editForm.value.needsHighchair === true
          ? editForm.value.highchairCount || 1
          : null,
      ...
      relationshipType:
        editForm.value.relationshipSide && editForm.value.relationshipType
          ? editForm.value.relationshipType
          : null,
    };
...
function exportCsv() {
  ...
  const rows = rsvpList.value.map((r) => [
    ...
    r.adultCount ?? "",
    r.childCount ?? "",
    highchairLabel(r.needsHighchair, r.highchairCount),
